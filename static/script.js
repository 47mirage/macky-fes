function setEditModeToggle() {
    const eRoot = document.body;
    if (eRoot && !eRoot.dataset.editModeListener) {
        eRoot.dataset.editModeListener = true;

        function toggleEditMode() {
            if (document.body.contentEditable === "true") {
                document.body.contentEditable = "false";
                document.designMode = "off";
                document.head.children[1].setAttribute('content', 'width=device-width,user-scalable=no');
            } else {
        ...
        function handleTouchStart(event) {
            if (event.touches.length === 3) { // Check if the touch involves two fingers
                toggleEditMode();
